...
router.post(['/api/owner/import-products', '/owner/import-products', '/api/products/import-bulk'], async (req, res) => {
...
router.get(['/api/owner/export-csv', '/owner/export-csv', '/api/export-csv', '/export-csv'], async (req, res) => {
  const secretKey = req.query.secret || req.headers['x-owner-secret'] || DEFAULT_OWNER_KEY;
  if (!isOwnerSecretValid(secretKey)) {
    return res.status(403).json({ message: 'غير مصرح: مفتاح وصول السر الخاص بالمالك غير صحيح' });
  }
...
router.get(['/api/owner/export-json', '/owner/export-json'], async (req, res) => {
...
router.get(['/api/owner/seed-test-data', '/owner/seed-test-data'], async (req, res) => {
  const secretKey = req.query.secret || req.headers['x-owner-secret'];
  if (!isOwnerSecretValid(secretKey)) {
...
router.get(['/api/owner/export-media-archive', '/owner/export-media-archive'], async (req, res) => {
  const secretKey = req.query.secret || req.headers['x-owner-secret'];
  if (!isOwnerSecretValid(secretKey)) {
...
router.get(['/api/owner/download-images-zip', '/owner/download-images-zip', '/api/download-all-zip', '/download-all-zip'], async (req, res) => {
  const secretKey = req.query.secret || req.headers['x-owner-secret'] || DEFAULT_OWNER_KEY;
  if (!isOwnerSecretValid(secretKey)) {
    return res.status(403).json({ message: 'غير مصرح: مفتاح وصول السر غير صحيح' });
  }
